...
/**
 * 结束异常辐射源定位信令帧,中心站转发至FPGA
 *
 * Created by Administrator on 2016/7/12
 */


public class stopPOAandTDOADecoder implements MessageDecoder {
    @Override
    ...
    public MessageDecoderResult decode(IoSession ioSession, IoBuffer in,
        ...
        while (in.hasRemaining()){
            ...
            if(hasCount==17){
                ...
                in.get(bytes);
                StopPOAandTDOA td=new StopPOAandTDOA();//余POA公用
                td.setContent(bytes);
                HandleSweepInfo(bytes);
                out.write(td);
                Log.d("stopPOAandTDOADecoder", Arrays.toString(bytes));
                return MessageDecoderResult.OK;
